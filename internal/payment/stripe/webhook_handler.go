...
func (h *handler) Handle(w http.ResponseWriter, req *http.Request) error {
	...
	return h.handleEvent(w, req, event)
}

func (h *handler) handleEvent(w http.ResponseWriter, req *http.Request, event stripe.Event) error {
	var paymentIntent stripe.PaymentIntent
	if err := json.Unmarshal(event.Data.Raw, &paymentIntent); err != nil {
		slog.Error("Error parsing payment intent", "error", err)
		return api.WrappedError(http.StatusBadRequest, "Failed to parse payment intent")
	}
	...
	if err != nil {
		slog.Error("Invalid transaction_id in webhook", "transaction_id", transactionID)
		return api.WrappedError(http.StatusBadRequest, "Invalid transaction_id")
	}
